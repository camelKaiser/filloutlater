#Winston Li 12-25-2017
#Handles the game simulation and feature extraction
#Features are as "intuitive" as possible, something is a feature only if it can be observed by a first-time chess player
...
def getFeatures():      #packages all the inputs used for the model
    input = []
    #feature order
    ...
    if board.turn: #White's move == true, Black's move == false
        ...
        for i in range(24):
            input += pieceList[i]
    return input
def getPieceList():         #piece-centric representation
